...
**Requested:** Jorge. Paired with Frodo on iOS.

**FIX 1 — Cabrini (and 26 others) tappable sources.**
Investigated Android side independently. `SaintDetailScreen.SourcesSection` (lines 319-357) already does the right thing: per-source row is made tappable via `LocalUriHandler.current.openUri(url)` when `saint.sourceURLs?.get(source)` resolves, with primary-color text + `OpenInNew` icon affordance; non-resolving sources render as plain onSurfaceVariant text (no bug). Root cause was pure data: 27 saints had `sources` arrays that didn't match their `sourceURLs` keys, so every lookup missed and nothing was tappable. Frodo fixed the canonical JSON in 7fb793c (synced `sources` to mirror `sourceURLs` keys across EN + ES). Android picks the fix up for free — assets get regenerated by `syncSharedContent` at build time, no Kotlin change needed.

**FIX 2 — Settings section order.**
`SettingsScreen.kt`: swapped the two trailing `item {}` blocks so `Support & Legal` (Privacy Policy / Support / Contact Us) appears before `Content Sources`. Content Sources is now 9 rows after Wikipedia + Wikimedia were added, which was pushing Privacy Policy off-screen on shorter devices. Commit 8532dd3.

**Verification:** `./gradlew :app:assembleDebug :app:testDebugUnitTest` → BUILD SUCCESSFUL. Only pre-existing deprecation warnings (AutoMirrored icon variants, HorizontalDivider rename) — untouched, unrelated to this change.
